  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <div className="glass-card p-5 rounded-2xl border border-slate-800">
          <span className="text-xs font-semibold text-sky-400 uppercase tracking-wider flex items-center gap-1.5"><Database className="w-3.5 h-3.5"/> DB Records</span>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-3xl font-extrabold text-white">{(summary.totalRecords || 0).toLocaleString()}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card p-6 rounded-3xl border border-slate-800 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-bold text-slate-100">Top Performing Sections</h2>
          </div>
          <div className="space-y-3">
            {(data?.topSections || []).map((sec, idx) => (
              <div key={sec.id} className="p-3 rounded-xl bg-slate-900/50 border border-slate-800 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-bold text-slate-200 text-sm">{sec.name}</div>
                    <div className="text-xs text-slate-400">{sec.instructor_name}</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="font-bold text-emerald-400">{sec.rate}%</div>
                  <div className="text-[10px] text-slate-500 uppercase tracking-wider">Attendance</div>
                </div>
              </div>
            ))}
            {(!data?.topSections || data.topSections.length === 0) && (
              <div className="text-center py-4 text-xs text-slate-500">No session data available.</div>
            )}
          </div>
        </div>

        {/* Recent Admin Audit Logs */}
        <div className="glass-card p-6 rounded-3xl border border-slate-800 space-y-4 flex flex-col h-full">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Activity className="w-5 h-5 text-sky-400" />
              <h2 className="text-base font-bold text-slate-100">Recent Admin Activity</h2>
            </div>
            <Link to="/admin/logs" className="text-xs text-sky-400 hover:text-sky-300">View All</Link>
          </div>
          <div className="flex-1 overflow-y-auto pr-2 space-y-4 relative">
            ...
            {(data?.auditLogs || []).map((log, idx) => (
              <div key={idx} className="relative pl-6">
                <div className="absolute left-[3px] top-1.5 w-1.5 h-1.5 rounded-full bg-sky-500 ring-4 ring-slate-950"></div>
                <div className="text-xs text-slate-500 mb-0.5">{new Date(log.timestamp).toLocaleString()}</div>
                <div className="text-sm font-semibold text-slate-300">{log.action}</div>
                <div className="text-xs text-slate-400 mt-1">{log.details}</div>
              </div>
            ))}
            {(!data?.auditLogs || data.auditLogs.length === 0) && (
              <div className="text-center py-4 text-xs text-slate-500 relative z-10 bg-slate-950">No recent activity.</div>
            )}
          </div>
        </div>
      </div>
