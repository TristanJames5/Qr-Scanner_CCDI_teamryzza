...
export function generateSectionAttendanceCSV(sectionId) {
  ...
  // FIXED N+1: Single batch query for all student attendance records across all sessions
  const sessionIdList = sessions.map(s => s.id);
  const allRecords = sessionIdList.length > 0
    ? db.prepare(`
  ...
  // Group by student_id in JS — zero DB calls in the student loop
  const recordsByStudent = new Map();
  for (const rec of allRecords) {
    if (!recordsByStudent.has(rec.student_id)) recordsByStudent.set(rec.student_id, new Map());
    recordsByStudent.get(rec.student_id).set(rec.session_id, rec.status);
  }

  for (const st of enrolled) {
    const recMap = recordsByStudent.get(st.id) || new Map();
