...
// S-Class: Geo-Fence Campus Boundaries (Placeholder for CCDI Sorsogon)
const CAMPUS_LAT = process.env.CAMPUS_LAT ? parseFloat(process.env.CAMPUS_LAT) : 12.9749;
const CAMPUS_LNG = process.env.CAMPUS_LNG ? parseFloat(process.env.CAMPUS_LNG) : 124.0058;
const MAX_DISTANCE_METERS = process.env.MAX_DISTANCE_METERS ? parseInt(process.env.MAX_DISTANCE_METERS) : 500;
...
router.post('/', authenticate, async (req, res) => {
  try {
    ...
    const { token, sessionId: directSessionId, backupCode, latitude, longitude } = req.body;
    
    // 0. S-Class: Geo-Fence Validation
    if (!latitude || !longitude) {
      return res.status(400).json({ error: 'Location required: Please enable GPS/Location Services on your device to scan.' });
    }
    
    const distance = calculateDistanceMeters(CAMPUS_LAT, CAMPUS_LNG, latitude, longitude);
    if (distance > MAX_DISTANCE_METERS) {
      return res.status(403).json({ 
        error: `Geo-Fence Active: You are ${Math.round(distance)} meters away from campus. You must be within ${MAX_DISTANCE_METERS}m to scan.` 
      });
    }
