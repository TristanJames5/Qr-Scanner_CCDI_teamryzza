    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
                      ...
                      <td className="px-5 py-3.5">
                        {log.status === 'absent' && (
                          <button
                            ...
                            className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-amber-500/20 text-amber-300 border border-amber-500/30 text-[11px] font-semibold hover:bg-amber-500/30 transition-colors"
                          >
      ...
      {excuseModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className="glass-card rounded-2xl border border-slate-700 p-6 w-full max-w-md space-y-4 shadow-2xl">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold text-slate-100 flex items-center gap-2">
                <FileText className="w-5 h-5 text-amber-400" />
                Submit Excuse Letter
              </h3>
              <button onClick={() => setExcuseModal(null)} className="text-slate-400 hover:text-slate-200">
                <X className="w-5 h-5" />
              </button>
            </div>

            <p className="text-xs text-slate-400">For: <span className="text-slate-200 font-medium">{excuseModal.subject}</span></p>

            {excuseSuccess ? (
              <div className="p-4 rounded-xl bg-emerald-950/60 border border-emerald-600/50 text-emerald-300 text-sm text-center">
              ...
              <>
                <div>
                  <label className="text-xs font-semibold text-slate-300 mb-1 block">Reason for Absence</label>
                  <textarea
                    value={excuseReason}
                    onChange={(e) => setExcuseReason(e.target.value)}
                    placeholder="e.g., Medical appointment, family emergency..."
                    className="w-full p-3 rounded-xl bg-slate-900 border border-slate-700 text-slate-200 text-sm placeholder-slate-500 focus:outline-none focus:border-blue-500 resize-none"
                    rows={3}
                  />
                </div>

                <div>
                  <label className="text-xs font-semibold text-slate-300 mb-1 block">Upload Supporting Document</label>
                  <label className="flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-slate-900 border border-dashed border-slate-600 text-slate-400 text-sm cursor-pointer hover:border-blue-500 hover:text-blue-300 transition-colors">
                    <Upload className="w-4 h-4" />
                    {excuseFile ? excuseFile.name : 'Click to upload (JPG, PNG, PDF — max 5MB)'}
                    <input
                      type="file"
                ...
                {excuseError && (
                  <p className="text-xs text-rose-400 flex items-center gap-1">
                    <AlertTriangle className="w-3.5 h-3.5" /> {excuseError}
                  </p>
                )}
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      try {
                        ...
                        // Fallback: if Cloudinary is not configured, upload a placeholder URL
                        sigData = null;
                      }
                      ...
                      // Step 3: Submit the excuse request to our backend
                  ...
                  className="w-full py-3 rounded-xl bg-amber-600 hover:bg-amber-500 disabled:bg-slate-700 disabled:text-slate-500 text-white font-bold text-sm flex items-center justify-center gap-2 transition-colors"
                >
                  {excuseSubmitting ? (
