...
| **Backend** | Node.js + Express.js + `helmet` + `express-rate-limit` | Event-driven RESTful API with HMAC-SHA256 token generation, security headers, and brute-force protection |
| **Database** | SQLite via Node.js built-in `DatabaseSync` (WAL Mode + composite indexes) | Zero-setup, self-contained relational database with ACID transactions and foreign keys |
| **Security** | JWT + bcrypt.js + HMAC-SHA256 + `crypto.timingSafeEqual` | Role-Based Access Control (`admin`, `instructor`, `student`) with timing-attack-resistant signed rotation |
...
```
┌──────────────────────────────┐   REST/JWT (HTTPS)   ┌─────────────────────────────────┐
│   React SPA (Vite)           │ ───────────────────► │  Node.js + Express API Server   │
│   Student Scanner View       │                       │  PORT 5000                      │
│   Instructor Projector HUD   │ ◄────────────────── │  • helmet (security headers)    │
│   Admin Dashboard            │   WebSocket           │  • CORS whitelist               │
│   PORT 5173 (dev)            │   (Socket.io)         │  • Rate limiting (auth + scan)  │
└──────────────────────────────┘                       └──────────────┬──────────────────┘
                                                                      │
                                                         ┌────────────▼────────────┐
                                                         │   SQLite (WAL Mode)     │
...
**Request flow for a student QR scan:**
`Student Camera → html5-qrcode → POST /api/scan (JWT) → verifyQRToken (HMAC + timingSafeEqual) → INSERT attendance_records → Socket.io broadcast → Instructor HUD updates live`

---

## 🔐 How the Anti-Proxy QR Engine Works

Each QR code encodes a **cryptographically signed token** with this structure:

```
base64({ s: sessionId, v: version, e: expiresAt, b: backupCode, sig: HMAC })
```

| Field | Meaning | Anti-Proxy Role |
| :--- | :--- | :--- |
| **`s`** | Session UUID | Token from Session A cannot be used in Session B |
| **`v`** | Version integer (increments every 30s) | Tokens older than 1 version are rejected — prevents screenshot replay |
| **`e`** | Unix ms expiry timestamp | Server validates `now < e + 5000ms` grace period for network lag |
| **`b`** | 6-char backup code (generated with `crypto.randomInt`) | Physical projector fallback — cryptographically secure RNG, not `Math.random()` |
| **`sig`** | First 32 hex chars of `HMAC-SHA256(s:v:e:b, QR_HMAC_SECRET)` | Verified with `crypto.timingSafeEqual()` to prevent [timing oracle attacks](https://codahale.com/a-lesson-in-timing-attacks/) |

**Why not GPS or Wi-Fi proximity?** Multi-story concrete university buildings cause ±50m GPS drift, trivially spoofable via Android Mock Location apps. Optical line-of-sight to a physical classroom projector is unforgeble. A shared screenshot expires within the 30-second rotation window.
